...
echo   1) SSH 스트리밍       (서버 설치 권한 없음, scripts\stream-logs.bat 사용)
...
:: config/alloy/alloy-config.alloy 생성 (Windows 호스트 경로 사용)
...
(
    ...
    echo $template = Get-Content "$scriptDir\config\alloy\alloy-config.host.template.alloy"
    ...
    echo }
    echo $out ^| Set-Content "$scriptDir\config\alloy\alloy-config.alloy" -Encoding UTF8
) > "%PS_TEMP%"
...
:journal
copy "%SCRIPT_DIR%\config\alloy\alloy-config-journal.template.alloy" "%SCRIPT_DIR%\config\alloy\alloy-config.alloy" >nul
...
:file
copy "%SCRIPT_DIR%\config\alloy\alloy-config-file.template.alloy" "%SCRIPT_DIR%\config\alloy\alloy-config.alloy" >nul
